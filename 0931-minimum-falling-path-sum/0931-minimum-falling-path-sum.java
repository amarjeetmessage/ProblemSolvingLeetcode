class Solution {
    public int minFallingPathSum(int[][] matrix) {
        ...
        for(int j = 0; j<n; j++) {
            int ans = solve(matrix, n, 0 ,j);
            mn = Math.min(mn, ans);
        }
        return mn;
    }
    public static int solve(int[][] matrix, int n, int i , int j) {
        if(j < 0 || j > n-1) return Integer.MAX_VALUE;
        if(i == n-1) return matrix[i][j];

        //RECURSIVE CALL
        int a = solve(matrix, n,i+1,j-1);
        int b = solve(matrix, n,i+1,j+1);
        int c = solve(matrix, n,i+1,j);

       return Math.min(c,Math.min(a,b)) + matrix[i][j];
