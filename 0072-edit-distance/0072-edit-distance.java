...
class Solution {
    ...
    public static int solve(String word1, String word2, int m, int n) {
        if(m < 0 && n < 0) return 0;
        if(m < 0 || n < 0) return Math.abs(m - n);
        ...
        if(word1.charAt(m) == word2.charAt(n)) {
            ...
            int ins = solve(word1,word2,m,n-1);
            int del = solve(word1,word2,m-1,n);
            int repl = solve(word1,word2,m-1,n-1);

            return 1+ Math.min(ins, Math.min(repl,del));
